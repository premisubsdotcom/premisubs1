...
async def handle_flow_email(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if allowed != ["ALL"] and email_addr.lower() not in [m.lower() for m in allowed]:
        return await update.message_reply(
...
if __name__ == "__main__":
    ...
    # Broadcast handlers:
    # 1) plain text /send <message>
    application.add_handler(CommandHandler("send", broadcast_from_admin))
    # 2) media with caption starting `/send`
